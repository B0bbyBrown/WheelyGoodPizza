...
export default function Sessions() {
  ...
  const calculateSessionStats = (session: any) => {
    const sessionSales = todaySales.filter((sale: any) => sale.sessionId === session.id);
    const cashSales = sessionSales.filter((sale: any) => sale.paymentType === 'CASH');
    const cardSales = sessionSales.filter((sale: any) => sale.paymentType === 'CARD');
    
    const totalSales = sessionSales.reduce((sum: number, sale: any) => sum + parseFloat(sale.total), 0);
    const cashTotal = cashSales.reduce((sum: number, sale: any) => sum + parseFloat(sale.total), 0);
    const cardTotal = cardSales.reduce((sum: number, sale: any) => sum + parseFloat(sale.total), 0);
  ...
  const getVariance = (session: any) => {
    if (!session.closedAt || !session.closingFloat) return 0;
    const expected = getExpectedCash(session);
    const actual = parseFloat(session.closingFloat);
        ...
        <CardContent>
          {activeSession ? (
            <div className="space-y-4" data-testid="active-session-info">
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-600">
                      Started: {formatDateTime(activeSession.openedAt)}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm text-green-600">Opening Float</p>
                  <p className="font-semibold text-green-800" data-testid="opening-float">
              ...
              {/* Session Statistics */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4" data-testid="session-stats">
                {(() => {
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-xl font-bold">{formatCurrency(stats.cardTotal)}</p>
                            </div>
              ...
              <Card className="bg-blue-50 border-blue-200">
                <CardContent className="p-4">
                  <div className="flex justify-between items-center">
                    <span className="font-medium text-blue-800">Expected Cash in Drawer:</span>
                    <span className="text-xl font-bold text-blue-800" data-testid="expected-cash">
              ...
              <Dialog open={isCloseDialogOpen} onOpenChange={setIsCloseDialogOpen}>
                <DialogTrigger asChild>
                  <Button className="w-full" variant="destructive" data-testid="close-session-button">
                ...
                <DialogContent data-testid="close-session-dialog">
                  ...
                  <div className="space-y-4">
                    ...
                    {closingFloat && (
                      <Card className={getVariance({ ...activeSession, closingFloat }) >= 0 ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}>
                        <CardContent className="p-3">
                          <div className="flex justify-between items-center">
                            <span className="font-medium">Variance:</span>
                            <span className={`font-bold ${getVariance({ ...activeSession, closingFloat }) >= 0 ? "text-green-600" : "text-red-600"}`}>
                              {getVariance({ ...activeSession, closingFloat }) >= 0 ? "+" : ""}
                              {formatCurrency(getVariance({ ...activeSession, closingFloat }))}
                            </span>
                          </div>
                        </CardContent>
                      </Card>
            ...
            <div className="space-y-4" data-testid="no-active-session">
              <div className="flex items-center justify-between p-4 bg-red-50 border border-red-200 rounded-lg">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="font-semibold text-red-800">No Active Session</h3>
              ...
              <Dialog open={isOpenDialogOpen} onOpenChange={setIsOpenDialogOpen}>
                <DialogTrigger asChild>
                  <Button className="w-full" data-testid="open-session-button">
        ...
        <CardContent>
          {sessionsLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground">Open your first session to start tracking</p>
            </div>
          ) : (
            <Table data-testid="sessions-table">
              ...
              <TableBody>
                {sessions.map((session: any, index: number) => {
                  ...
                  return (
                    <TableRow key={session.id} data-testid={`session-row-${index}`}>
                      <TableCell>
                      ...
                      <TableCell>{formatCurrency(session.openingFloat)}</TableCell>
                      <TableCell>{formatCurrency(stats.totalSales)}</TableCell>
                      <TableCell>{formatCurrency(getExpectedCash(session))}</TableCell>
                      <TableCell>
                        {session.closingFloat ? formatCurrency(session.closingFloat) : "-"}
                      </TableCell>
                      <TableCell>
